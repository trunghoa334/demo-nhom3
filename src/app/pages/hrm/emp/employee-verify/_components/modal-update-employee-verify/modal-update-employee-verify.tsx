...
import { UpdateEmployeeVerifyRequestType } from '~/app/types/employee-verify/request/employee-verify.type'
...
interface ModalUpdateEmployeeVerifyProps {
    id: number
    modal: boolean
    toggle: () => void
}

export default function ModalUpdateEmployeeVerify({ id, modal, toggle }: ModalUpdateEmployeeVerifyProps) {
    const { getLangKey } = useLang()
    const { messageSuccess } = useToastMessageAsync()

    const { data: employeeVerify } = useQuery<EmployeeVerifyType>({
        queryKey: [TANSTACK_KEY.EMOLOYEE_VERIFY_ONE, id],
        queryFn: () => employeeVerifyQueryApi.getEmployeeVerifyById(id),
        enabled: !!id
    })
    const selectedBirthDate = useMemo(() => formatDateTimestamp(employeeVerify?.verCreatedDate ?? null), [employeeVerify])
    ...
    const {
        ...
        mutationFn: ({ id, body }: { id: number; body: UpdateEmployeeVerifyRequestType }) =>
            employeeVerifyCommandApi.updateEmployeeVerify(id, body)
    })
    ...
    const {
        ...
        setValue,
        formState: { errors }
    } = useForm<UpdateEmployeeVerifyRequestType>({
    ...
    const onSubmit: SubmitHandler<UpdateEmployeeVerifyRequestType> = (data) => {
        mutate(
            { id, body: data },
            {
                onSuccess: () => {
                    queryClient.refetchQueries({ queryKey: [TANSTACK_KEY.EMOLOYEE_VERIFY_ALL] })
                    queryClient.invalidateQueries({ queryKey: [TANSTACK_KEY.EMOLOYEE_VERIFY_ONE, id] })
                    toggle()
    ...
    const handleCreatedDate = (date: Date | null) => {
        if (date) {
            const timestampInSeconds = Math.floor(date.getTime() / 1000)
            setValue('verCreatedDate', timestampInSeconds, { shouldValidate: true })
        }
    }
    ...
    return (
        <ModalCommon
            modal={modal}
            onClose={() => toggle()}
            title={getLangKey(CONFIG_LANG_KEY.PAGE_EMPLOYEE_VERIFY_UPDATE_EV)}
            ...
            <Form innerRef={formRef} onSubmit={handleSubmit(onSubmit)}>
                ...
                <Row>
                        ...
                        <div className='text-center'>
                            <div className='position-relative d-inline-block'>
                                <ImageUpload
                                    name='verImageBase64'
                                    errors={errors}
                                    imageURL={employeeVerify?.verImage}
                    ...
                    <Col lg={6}>
                        ...
                        <SelectSingle
                            selected={String(employeeVerify?.employee.id)}
                            name='employeeId'
                            errors={errors}
                    ...
                    <Col lg={6}>
                        <Label>{getLangKey(CONFIG_LANG_KEY.PAGE_EMPLOYEE_VERIFY_TITLE_INPUT_DATE_VERIFY)}</Label>
                        <DatePicker
                            name='verCreatedDate'
                            errors={errors}
